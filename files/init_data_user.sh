...
[ -z "${SEAFILE_VERSION}" ] && SEAFILE_VERSION="6.2.2"
...
if [ -n "${SEAFILE_PORT}" ]; then	
    SEAFILE_SERVICE="${SEAFILE_HOST}:${SEAFILE_PORT}"
fi

if [ "${SEAFILE_USE_HTTPS}" == "1" ]; then 
    SEAFILE_SERVICE="https://${SEAFILE_SERVICE}" 
else
    SEAFILE_SERVICE="http://${SEAFILE_SERVICE}"
fi
